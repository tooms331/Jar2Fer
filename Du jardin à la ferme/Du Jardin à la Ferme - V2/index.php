...
$tomate_cerise = $bdd->Produits_Ajouter("Tomate cerise","Des toute pitite tomat'");
echo json_encode($tomate_cerise,JSON_PRETTY_PRINT| JSON_UNESCAPED_UNICODE);

$concombre = $bdd->Produits_Ajouter("concombre","concomb'");
echo json_encode($concombre,JSON_PRETTY_PRINT| JSON_UNESCAPED_UNICODE);

$variation = $bdd->VariationsStock_Ajouter($tomate_cerise->id_produit, 10, 'RECOLTE','Premire rcolte :-)');
echo json_encode($variation,JSON_PRETTY_PRINT| JSON_UNESCAPED_UNICODE);

$variation = $bdd->VariationsStock_Ajouter($concombre->id_produit, 15, 'RECOLTE','le concombre masqu!');
echo json_encode($variation,JSON_PRETTY_PRINT| JSON_UNESCAPED_UNICODE);

$commande = $bdd->Commande_Récupéré_Creer_Creation($compte->id_compte);
